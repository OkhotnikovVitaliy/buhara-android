 ...
 *
 */
class MenuRepo(private val api: Api) {

    suspend fun fetchCategories(idRestaurant: Int): Response<ArrayList<CategoryModel>>? {
        return try {
            return api.fetchCategories(idRestaurant)
        }catch (e:Exception){
            null
        }
    }
